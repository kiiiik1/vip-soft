...
class VIPSoftPromoter:
    ...
    def _generate_github_template(self):
        """生成GitHub推广模板"""
        content = f"""## {self.promotion_data['project_name']}

{self.promotion_data['description']}
...
{self.promotion_data['call_to_action']}

{self.promotion_data['contact_info']}

---
*推广时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*
"""
        return content
        
    def _generate_weibo_template(self):
        """生成微博推广模板"""
        content = f"""【{self.promotion_data['project_name']}】开源项目推荐！
...
🔗 项目地址：{self.promotion_data['project_url']}
{self.promotion_data['call_to_action']}
...
"""
    ...
    def _generate_zhihu_template(self):
        """生成知乎推广模板"""
        content = f"""有哪些实用的技术工具推荐？我推荐【{self.promotion_data['project_name']}】
...
## 项目地址
{self.promotion_data['project_url']}
...
"""
    ...
    def _generate_v2ex_template(self):
        """生成V2EX推广模板"""
        content = f"""【分享】{self.promotion_data['project_name']} - 开源技术工具集
...
URL: {self.promotion_data['contact_info']}

---
Promoted on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
"""
        return content
